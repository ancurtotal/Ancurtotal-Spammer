#!/data/data/com.termux/files/usr/bin/bash
# Auto Install Tools Spammer
# Version SODC v1

#HayoMauNgapain??
#DiRecodeGakBakalJadiinGueNewbie
#DiGantiAuthorGakBakalJadiinGueNoob
#MikirPakeDengkulBroJanganPakeOtak
#JanganReedit/RecodeYaKecualiKaloLuGakAdaOtaknya
#BelajarBarengYukBro??

clear
cd module
python2 ancurtotal.py
# Variables
b='\033[1m'
u='\033[4m'
bl='\E[30m'
r='\E[31m'
g='\E[32m'
bu='\E[34m'
m='\E[35m'
c='\E[36m'
w='\E[37m'
endc='\E[0m'
enda='\033[0m'
blue='\e[1;34m'
cyan='\e[1;36m'
red='\e[1;31m'

figlet ANCURTOTAL | lolcat
echo "                 #  SCARY OF DEMON CYBER # " | lolcat
echo "_____________________________________________________________" | lolcat
echo
echo "TYPE       : AUTO INSTALLER TOOLS SPAMMER $green " |lolcat
echo "VERSION    : SODC v1 " | lolcat
echo "TOTALS     : 20 TOOLS SPAM " | lolcat
echo "AUTHOR     : EKAY ANCURTOTAL $green " | lolcat
echo "TEAM       : SCARY OF DEMON CYBER $green " | lolcat
echo "FROM       : INDONESIA $green " | lolcat
echo "THANKS YOU : ALL MEMBER SODC TEAM " | lolcat
echo "NOTES      : JANGAN DI PAKAI BUAT NGERJAIN ORANG " | lolcat
echo "_____________________________________________________________" | lolcat

sleep 1

###################################################
# CTRL + C
###################################################
trap ctrl_c INT
ctrl_c() {
clear
echo "Thanks To :" | lolcat
echo "- Allah SWT" | lolcat
echo "- My Little Family" | lolcat
echo "- My Wife, Mama Fitri Ndut" | lolcat
echo "- My Two Daughters, Fieka and Anin" | lolcat
echo "- M-1971 SODC" | lolcat
echo "- Mr.Bus1Dho SODC" | lolcat
echo "- Mr.D43NGROOT SODC" | lolcat
echo "- Eteus SODC" | lolcat
echo "- Bunga SODC" | lolcat
echo "- All Member SODC Team" | lolcat
echo
echo "Scary Of Demon Cyber On Your System" | lolcat
echo "Ekay Ancurtotal Is In Your Heart :)" | lolcat
echo "See You Again BACOT SKILLS😎 :)..." | lolcat
echo
echo "Thanks You For Using My Tools" | lolcat
echo "SEMUA TOOLS YANG KALIAN INSTALL ADA DI module TOOLS" | lolcat
figlet ANCURTOTAL | lolcat
echo "                 #  SCARY OF DEMON CYBER # " | lolcat
sleep 1
exit
}


lagi=1
while [ $lagi -lt 6 ];
do

echo ""
echo ""

echo "######################################" | lolcat
echo "########  ANCURTOTAL SPAMMER  ########" | lolcat
echo "######################################" | lolcat

echo ""
echo "============================" | lolcat
echo $b "1. Install LITESPAM${enda}";
echo "============================" | lolcat
echo $b "2. Install bom${enda}";
echo "============================" | lolcat
echo $b "3. Install Bom Sms 3${enda}";
echo "============================" | lolcat
echo $b "4. Install Santet-Online${enda}";
echo "============================" | lolcat
echo $b "5. Install CnkSpam${enda}";
echo "============================" | lolcat
echo $b "6. Install RusSpam${enda}";
echo "============================" | lolcat
echo $b "7. Install SpazSMS${enda}";
echo "============================" | lolcat
echo $b "8. Install New-Spammer${enda}";
echo "============================" | lolcat
echo $b "9. Install Spam${enda}";
echo "============================" | lolcat
echo $b "10. Install QJDID${enda}";
echo "============================" | lolcat
echo $b "11. Install QFloodSms${enda}";
echo "============================" | lolcat
echo $b "12. Install Spam-Mantan${enda}";
echo "============================" | lolcat
echo $b "13. Install AVARSpam${enda}";
echo "============================" | lolcat
echo $b "14. Install ANRSpam${enda}";
echo "============================" | lolcat
echo $b "15. Install MultiSpam${enda}";
echo "============================" | lolcat
echo $b "16. Install SpamChat${enda}";
echo "============================" | lolcat
echo $b "17. Install GCOspam${enda}";
echo "============================" | lolcat
echo $b "18. Install Spammer-Grab${enda}";
echo "============================" | lolcat
echo $b "19. Install Spam Boms Tsel${enda}";
echo "============================" | lolcat
echo $b "20. Install Spam Boms Mall${enda}";
echo "============================" | lolcat
echo $b "00. Exit${enda}";
echo "============================" | lolcat
echo "     [PILIH NOMORNYA BRO]"
echo
read -p "SODC@ancurtotal~#" pil;

#Install LITESPAM

case $pil in
1) clear
echo "Install LITESPAM...Installing, Please Wait !!" | lolcat
sleep 1
pkg update
pkg upgrade
pkg install python2
pkg install php
pkg install toilet
pkg install sh
pkg install git
git clone https://github.com/4L13199/LITESPAM
cd LITESPAM
sh LITESPAM.sh


;;

#Install Bom

2) clear
echo "Install bom...Installing, Please Wait !!" | lolcat
sleep 1 
pkg update
pkg upgrade
pkg install python2
pkg install php
pkg install git
git clone https://github.com/rootnet007/bom
cd bom
php bom.php


;;

#Install Bom Sms 3

3) clear
echo "Install Bom Sms 3...Installing, Please Wait !!" | lolcat
sleep 1
pkg update
pkg upgrade
pkg install python2
pkg install php
pkg install sh
pkg install git
git clone https://github.com/ardzz/tri/
echo "SUDAH TERINSTALL BRO"


;;

#Install Santet-Online

4) clear
echo "Install Santet-Online...Installing, Please Wait !!" | lolcat
sleep 1
apt update
apt upgrade
apt install python2
apt install git
git clone https://github.com/Gameye98/santet-online
cd santet-online
chmod +x santet.py
python2 santet.py


;;

#install CnkSpam

5) clear
echo "Install CnkSpam...Installing, Please Wait !!" | lolcat
sleep 1
pkg update
pkg upgrade
pkg install python2
pkg install php
pkg install sh
pkg install git
git clone https://github.com/hatakecnk/cnkspam
echo "SUDAH TERINSTALL BRO"


;;

#Install RusSpam

6) clear
echo "Install RusSpam...Installing, Please Wait !!" | lolcat
sleep 1
pkg update
pkg upgrade
pkg install python2
pkg install php
pkg install sh
pkg install git
git clone https://github.com/Rusmana-ID/rus
echo "SUDAH TERINSTALL BRO"


;;

#Install SpazSMS

7) clear
echo "Install SpazSMS...Installing, Please Wait !!" | lolcat
sleep 1
pkg update
pkg upgrade
pkg install python2
pkg install php
pkg install sh
pkg install git
git clone https://github.com/Gameye98/SpazSMS
echo "SUDAH TERINSTALL BRO"


;;

# Install New-Spammer

8) clear
echo "Install New-Spammer...Installing, Please Wait !!" | lolcat
sleep 1
pkg update
pkg upgrade
pkg install python2
pkg install php
pkg install sh
pkg install git
git clone https://github.com/haijuga7/New-Spammer
echo "SUDAH TERINSTALL BRO"


;;

#Install Spam

9) clear
echo "Install Spam...Installing, Please Wait !!" | lolcat
sleep 1
pkg update
pkg upgrade
pkg install python2
pkg install php
pkg install sh
pkg install git
git clone https://github.com/SIIL3NT/spam
echo "SUDAH TERINSTALL BRO"


;;

#Install QJDID

10) clear
echo "Install QJDID...Installing, Please Wait !!" | lolcat
sleep 1
pkg update
pkg upgrade
pkg install python2
pkg install php
pkg install sh
pkg install git
git clone https://github.com/QiubyZ/QJDID
echo "SUDAH TERINSTALL BRO"


;;

#Install QFloodSms

11) clear
echo "Install QFloodSms...Installing, Please Wait !!" | lolcat
sleep 1
pkg update
pkg upgrade
pkg install python2
pkg install php
pkg install sh
pkg install git
git clone https://github.com/QiubyZ/QFloodSms
echo "SUDAH TERINSTALL BRO"


;;

#Install Spam-Mantan

12) clear
echo "Install Spam-Mantan...Installing, Please Wait !!" | lolcat
sleep 1
pkg update
pkg upgrade
pkg install python2
pkg install php
pkg install sh
pkg install git
git clone https://github.com/not404foundcyberteam/spam-mantan
echo "SUDAH TERINSTALL BRO"


;;

#Install AVARspam

13) clear
echo "Install AVARspam...Installing, Please Wait !!" | lolcat
sleep 1
pkg update
pkg upgrade
pkg install python2
pkg install php
pkg install sh
pkg install git
git clone https://github.com/ALX-04/AVARspam
echo "SUDAH TERINSTALL BRO"


;;

#Install ANRspamm

14) clear
echo "Install ANRspam...Installing, Please Wait !!" | lolcat
sleep 1
pkg update
pkg upgrade
pkg install python2
pkg install php
pkg install sh
pkg install git
git clone https://github.com/Amriez/ANRspam
echo "SUDAH TERINSTALL BRO"


;;

#Install MultiSpam

15) clear
echo "Install MultiSpam...Installing, Please Wait !!" | lolcat
sleep 1
pkg update
pkg upgrade
pkg install python2
pkg install php
pkg install sh
pkg install git
git clone https://github.com/kereh/MultiSpam
echo "SUDAH TERINSTALL BRO"


;;

#Install SpamChat

16) clear
echo "Install SpamChat...Installing, Please Wait !!" | lolcat
sleep 1
pkg update
pkg upgrade
pkg install python2
pkg install php
pkg install sh
pkg install git
git clone https://github.com/Senitopeng/Spamchat
echo "SUDAH TERINSTALL BRO"


;;

#Install Spam GCOspam

17) clear
echo "Install Spam GCOspam...Installing, Please Wait !!" | lolcat
sleep 1
apt update
apt upgrade
apt install python2
pkg install php
pkg install sh
apt install git
git clone https://github.com/Amriez/gcospam
cd gcospam
sh install.sh
sh gco.sh


;;

#Install Spammer-Grab

18) clear
echo "Install Spammer-Grab...Installing, Please Wait !!" | lolcat
sleep 1
pkg update
pkg upgrade
pkg install python2
pip2 install requests
pkg install git
git clone https://github.com/p4kl0nc4t/Spammer-Grab
Spammer-Grab
chmod +x spammer.py
python2 spammer.py


;;

#Install Spam Boms Tsel

19) clear
echo "Install Spam Boms Tsel...Installing, Please Wait !!" | lolcat
sleep 1
pkg update
pkg upgrade
pkg install curl
pkg install php
curl -s http://files-store.theprivat.ml/uploads/bombtsel.txt > bombtsel.php 
chmod 777 bombtsel.php
php bombtsel.php


;;

#Install Spam Bom Mall

20) clear
echo "Install Spam Bom Mall...Installing, Please Wait !!" | lolcat
pkg install update
pkg install upgrade
pkg install wget
pkg install php
wget http://files-store.theprivat.ml/uploads/bom-mall.zip
unzip bom-mall.zip
cd bom-mall
php run.php


;;

00) echo "Thanks To :" | lolcat
echo "- Allah SWT" | lolcat
echo "- My Little Family" | lolcat
echo "- My Wife, Mama Fitri Ndut" | lolcat
echo "- My Two Daughters, Fieka and Anin" | lolcat
echo "- M-1971 SODC" | lolcat
echo "- Mr.Bus1Dho SODC" | lolcat
echo "- Mr.D43NGROOT SODC" | lolcat
echo "- Eteus SODC" | lolcat
echo "- Bunga SODC" | lolcat
echo "- ALL MEMBER SODC TEAM" | lolcat
echo
echo "Scary Of Demon Cystem On Your System" | lolcat
echo "Ekay Ancurtotal Is In Your Heart :)" | lolcat
echo "See You Again BACOT SKILLS😎 :)..." | lolcat
echo
echo "Thanks You For Using My Tools" | lolcat
echo "SEMUA TOOLS YANG KALIAN INSTALL ADA DI module TOOLS" | lolcat
figlet ANCURTOTAL | lolcat
echo "                 #  SCARY OF DEMON CYBER # " | lolcat
exit


;;

*) echo "Sorry, Your choices it's not already [T4T]"
esac
done
done

